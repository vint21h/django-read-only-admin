...
from read_only_admin.settings import PERMISSION_PREFIX, PERMISSION_NAME_PREFIX


__all__ = ["add_readonly_permissions"]
...
def add_readonly_permissions(sender, *args, **kwargs):
    ...
    for content_type in ContentType.objects.all():
        codename = "{prefix}_{model}".format(
            **{"prefix": PERMISSION_PREFIX, "model": content_type.model}
        )
        name = "{prefix} {model}".format(
            **{"prefix": PERMISSION_NAME_PREFIX, "model": content_type.model}
        )
